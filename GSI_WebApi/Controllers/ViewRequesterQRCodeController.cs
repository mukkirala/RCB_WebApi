...
{
    public class ViewRequesterQRCodeController : ApiController
    {
        [HttpPost]
        ...
        public List<ViewRequesterQRCodeModel> Get(ViewRequesterQRCodeModel data)
        {
            ...
            using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["NRLSAPConnectionString"].ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.CommandText = "Select AssetID,MainAssetNumber,AssetSubNumber,CustodianDepartment,AssetDesc,AdditionalDesc,Unit,Quantity,AssetCapitalizationDate,FirstAcquisitionDate,CustodianID,Location,LocationDesc,Status,StatusDesc,AssetClass,Component,ComponentDesc,Deacdate,Invdate,InventoryNote,QRImage,QRCode,Latitude,Longitude From RCBSAP..vAssetMaster where CustodianID ='" + data.EmployeeID + "' and Status !='NVAL'";
                    conn.Open();
