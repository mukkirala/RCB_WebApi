...
{
    public class DepWiseAuditsController : ApiController
    {
        [HttpPost]
        public List<AuditMaster> Get(AuditMaster data)
        {
            ...
            using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["VisitorConnectionString"].ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.CommandText = "SELECT [AuditID], [AuditName], [AuditBy], AuditMaster.[LocationID],Location FROM [AuditMaster]  inner join LocationMaster on LocationMaster.LocationID = AuditMaster.LocationID WHERE(AuditMaster.[LocationID] = '" + data.LocationID + "')  and(AuditMaster.AuditStatus = 'Active') and CustodianDepartment = '" + data.CustodianDepartment + "'";
